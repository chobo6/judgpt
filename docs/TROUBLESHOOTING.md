...
**해결**: 아직 미해결 — 이번엔 원인 조사만 하고 프롬프트/모델을 바꾸진 않았다. 후속 작업 후보:
- `prompt.py`에 협박 판단 기준(암시적 위협 포함 여부 등)을 명시적으로 추가하고 재측정.
- `OllamaLLM.call()`에 `temperature=0`(또는 낮은 값) 옵션을 추가해 eval 실측의 재현성을 높이는 것을 고려 — 단, 실제 사용 시(`analyze.py`)에는 낮은 temperature가 오히려 안 좋을 수도 있어(다양한 표현 포착) eval 전용으로만 낮출지 고민 필요.
- `exaone3.5:7.8b` 대 `qwen2.5:7b` 비교는 여전히 미실측(`docs/02-architecture.md` §10에 남아있던 질문) — 이제 eval 하네스가 있으니 두 모델로 각각 돌려서 비교 가능.
