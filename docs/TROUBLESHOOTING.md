...
**결론**: exaone3.5는 명예훼손·협박에, qwen2.5는 욕설·성적 발언에 더 강하다. 어느 한쪽이 전반적으로 우월하다고 보기 어렵고, 둘 다 성희롱(거부 이후 반복 판단)은 전혀 못 잡는다 — 이건 모델 문제라기보다 `prompt.py`의 성희롱 판단 기준 자체가 약해서일 가능성이 높다(협박처럼 구체적 판별 기준을 보강하면 개선될 여지). 기본 모델은 일단 `exaone3.5:7.8b`로 유지 — 협박(신변 위협)을 놓치는 게 성적 발언을 놓치는 것보다 실사용 리스크가 크다고 판단. **후속 작업**: 성희롱 판단 기준을 협박과 같은 방식으로 구체화하고 재측정.
